...
To implement geologically distributed cache : my idea was to make seperated cache for each region and access the cache based on the incoming IP address. In the code I have made seperate cache objects for 3 regions which can be stored in different servers, and can be accessed based on their IP, for simplicity I have filterd them using the first octate of the requesting IP address. Mapping of ocatate to region is given at IP_REGION 
